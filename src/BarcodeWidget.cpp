...
: QWidget(parent)
{
    ...
    filePathEdit->setStyleSheet("QLineEdit { border: 1px solid #ccc; border-radius: 5px; padding: "
                                "5px; background-color: #f9f9f9; }");

    QPushButton* browseButton = new QPushButton("浏览", this);
    browseButton->setFixedWidth(100);
    browseButton->setFont(QFont("Arial", 16));
    browseButton->setStyleSheet("QPushButton { background-color: #4CAF50; color: white; "
    ...
    // 可选：设置一点样式让它更好看
    progressBar->setStyleSheet("QProgressBar { border: 1px solid #ccc; border-radius: 5px; "
    ...
    widthInput->setStyleSheet("QLineEdit { border: 1px solid #ccc; border-radius: 5px; padding: "
                              "5px; background-color: #f9f9f9; }");
    ...
    heightInput->setStyleSheet("QLineEdit { border: 1px solid #ccc; border-radius: 5px; padding: "
                               "5px; background-color: #f9f9f9; }");
    ...
    fileDialog = new QFileDialog(
        this, "Select File", "", "Supported Files (*.rfa *.txt *.png);;All Files (*)");
    ...
    connect(
        decodeToChemFile, &QPushButton::clicked, this, &BarcodeWidget::onDecodeToChemFileClicked);
...
void BarcodeWidget::onGenerateClicked()
{
    ...
    if (directTextAction->isChecked())
    {
        ...
        // 启动异步任务
        watcher->setFuture(
            QtConcurrent::mapped(inputs, TextWorker{useBase64, {reqWidth, reqHeight, format}}));
    ...
    watcher->setFuture(
        QtConcurrent::mapped(filePaths, worker{reqWidth, reqHeight, useBase64, format}));
}
            ...
            try
            {
                ...
                case convert::result_i2t::invalid_qrcode:
                    ...
                    return {std::move(path),
                            QString{"无法加载图片文件: %1"}.arg(path).toStdString()};
...
void BarcodeWidget::onSaveClicked()
{
            ...
            overload_def_noop{
                ...
                [&](const QImage&) {
                    return QFileDialog::getSaveFileName(
                        this, "保存图片", defName, "PNG Images (*.png)");
                },
                [&](const QByteArray&) {
                    return QFileDialog::getSaveFileName(
    ...
    struct worker
    {
        ...
        SaveResult operator()(const SaveTask& task) const noexcept
        try
        {
    ...
    connect(watcher,
            &QFutureWatcher<SaveResult>::progressValueChanged,
            progressBar,
            &QProgressBar::setValue);
...
void BarcodeWidget::showMqttDebugMonitor() const { messageWidget->show(); }
...
void BarcodeWidget::renderResults() const
{
    ...
    if (lastResults.empty())
    {
        if (!lastSelectedFiles.empty())
        {
            ...
            QLabel* headerLabel =
                new QLabel(QString("已选择 %1 个文件，准备处理:").arg(lastSelectedFiles.size()));
            headerLabel->setStyleSheet(
                "font-weight: bold; font-size: 14px; color: #555; margin-bottom: 5px;");
            ...
            for (const QString& filePath : lastSelectedFiles)
            {
                ...
                nameLabel->setStyleSheet("border: none; background: transparent; font-family: "
                                         "Consolas; font-size: 14px; color: #333;");
                ...
                if (isImage)
                {
                    ...
                    typeLabel->setStyleSheet(typeLabel->styleSheet() +
                                             "color: #67C23A;"); // 橙色提示解码
                }
                else if (isText)
                {
        ...
        std::visit(
            overload_def_noop{
                ...
                [&](const QByteArray& data) {
                    ...
                    textLabel->setStyleSheet("border: 1px solid #ddd; background: white; padding: "
                                             "15px; font-family: Consolas; font-size: 14pt;");
                    textLabel->setSizePolicy(QSizePolicy::Preferred,
                                             QSizePolicy::Preferred); // 允许内容撑开
                    contentWidget = textLabel;
                },
                [&](const std::string& str) {
                    QLabel* errLabel = new QLabel(QString::fromStdString(str));
                    errLabel->setStyleSheet("color: red; border: 1px solid red; background: "
                                            "#fff0f0; padding: 15px; font-size: 14pt;");
            ...
            std::visit(
                overload_def_noop{
                    ...
                    [&](const QByteArray& data) {
                        ...
                        textLabel->setStyleSheet("border: 1px solid #ddd; background: white; "
                    ...
                    [&](const std::string& str) {
                        QLabel* errLabel = new QLabel(QString::fromStdString(str));
                        errLabel->setStyleSheet("font-size: 15pt; color: red; border: 1px solid "
                                                "red; background: #fff0f0; padding: 5px;");
                        errLabel->setWordWrap(true);
...
template <> struct magic_enum::customize::enum_range<ZXing::BarcodeFormat>
{
